...
const jest_atomic_1 = require("jest-atomic");
// TestScope :: START
const IconRegistry_1 = require("./IconRegistry");
const mockExtractorSync_1 = require("./mockExtractorSync");
const mockExtractor_1 = require("./mockExtractor");
const reg = new IconRegistry_1.IconRegistry();
// TestScope :: END
const testRegistry = new jest_atomic_1.AtomicTestRegistry();
// ALL TESTS :: START
// ITERATION 0 :: Test 0 - 6 :: Added 2019-07-28
{
    testRegistry.addTest(0, 'Add icon not implement', () => {
        expect(reg.addIconSync('', '', '')).toStrictEqual(Error('No implemented yet'));
    });
    testRegistry.addTest(1, 'Remove icon implement', () => {
        expect(reg.removeIconSync('', '', '')).toStrictEqual(Error('No implemented yet'));
    });
    ...
    testRegistry.addTest(5, 'Add plugin not implement', () => {
        expect(reg.addPlugin('test', {
            extractor: mockExtractor_1.mockExtractor,
            extractorSync: mockExtractorSync_1.mockExtractorSync
        })).toStrictEqual(Error('No implemented yet'));
    });
    testRegistry.addTest(6, 'Remove plugin not implements', () => {
        expect(reg.removePlugin('test')).toStrictEqual(Error('No implemented yet'));
    });
}
...
// ITERATION 2 :: Test 8 :: Added 2019-07-28
{
    testRegistry.addTest(8, 'Implementation of Remove Plugin', () => {
        ...
        expect(reg.removePlugin('test')).toStrictEqual(Error('Plugin not exist in registry'));
...
// ITERATION 3 :: Test 9 :: Added 2019-07-28
{
    testRegistry.addTest(9, 'Implementation of Add Icon Sync', () => {
        ...
        expect(reg.addIconSync('never', 'some', 'alert')).toStrictEqual(Error('Incorrect Set'));
        expect('test' in reg.plugins).toBe(true);
        expect(reg.addIconSync('test', 'primary', 'alert')).toBe(true);
        expect('test__primary__alert' in reg.icons).toBe(true);
        expect(reg.addIconSync('test', 'primary', 'alert')).toStrictEqual(Error('Icon is already added'));
        expect(reg.addIconSync('test', 'never', 'alert')).toStrictEqual(Error('Icon not exist in this set and/or group'));
    });
}
// ITERATION 4 :: Test 10 :: Added 2019-07-28
{
    testRegistry.addTest(10, 'Implementation of Remove Icon Sync', () => {
        ...
        expect(reg.removeIconSync('test', 'primary', 'alert')).toStrictEqual(Error('Icon not exist in registry'));
    });
}
// ITERATION 5 :: Test 11 - 12 :: Added 2019-07-28
{
    testRegistry.addTest(11, 'Implementation of Get Icon Sync', () => {
        ...
        expect(reg.getIconSync('test', 'primary', 'alert')).toStrictEqual([
            undefined,
            {
                content: 'M10.01 21.01c0 1.1.89 1.99 1.99 1.99s1.99-.89 1.99-1.99h-3.98zm8.87-4.19V11c0-3.25-2.25-5.97-5.29-6.69v-.72C13.59 2.71 12.88 2 12 2s-1.59.71-1.59 1.59v.72C7.37 5.03 5.12 7.75 5.12 11v5.82L3 18.94V20h18v-1.06l-2.12-2.12zM16 13.01h-3v3h-2v-3H8V11h3V8h2v3h3v2.01z',
                viewBox: [0, 0, 24, 24]
            }
        ]);
...
// ITERATION 6 :: Test 13 :: Added 2019-07-28
{
    testRegistry.addTest(13, 'Implementation of Compile Sprite', () => {
        ...
        expect(reg.compileSprite()).toBe('<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><symbol id="test__primary__alert" viewBox="0 0 24 24"><path d="M10.01 21.01c0 1.1.89 1.99 1.99 1.99s1.99-.89 1.99-1.99h-3.98zm8.87-4.19V11c0-3.25-2.25-5.97-5.29-6.69v-.72C13.59 2.71 12.88 2 12 2s-1.59.71-1.59 1.59v.72C7.37 5.03 5.12 7.75 5.12 11v5.82L3 18.94V20h18v-1.06l-2.12-2.12zM16 13.01h-3v3h-2v-3H8V11h3V8h2v3h3v2.01z"/></symbol></svg>');
    });
}
// ALL TESTS :: END
...
toTest = jest_atomic_1.iterate({
    ...
    outdateTest: [],
    newTest: [8],
    iterationDescription: '002 Nothing not implement, except Remove & Add plugin'
});
toTest = jest_atomic_1.iterate({
    testRegistryShadow: testRegistry,
    currentIterate: ITERATION,
